...
from ai_cdn.schemas.blueprint import ResourceDefinition, SystemBlueprint

logger = logging.getLogger(__name__)

# Standard label to be applied to all resources managed by AI-CDN
AI_CDN_BLUEPRINT_LABEL = "ai-cdn-blueprint"
...
class KubernetesEngine(Engine):
    ...
    async def get_state(self, blueprint: SystemBlueprint) -> List[ResourceState]:
        ...
        """
        await self._initialize_clients()
        label_selector = f"{AI_CDN_BLUEPRINT_LABEL}={blueprint.name}"
    ...
    def _construct_deployment(self, resource: ResourceDefinition) -> client.V1Deployment:
        ...
        name = resource.name
        labels = {
            AI_CDN_BLUEPRINT_LABEL: resource.metadata.get("blueprint_name", "unknown"),
            "app": name
        }
